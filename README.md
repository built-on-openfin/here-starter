...
This branch covers version **v15.0** of Here (there are versioned branches for other releases).
...
| [How To Use The Cloud Notification Service](./how-to/use-notifications-service/)   | This is a collection of examples demonstrating how to manage notifications in the HERE Notification Studio using the Cloud Notification Service API. Included are both Typescript and Java examples.  |
| [How To Use The GraphQL API For App and User Management](./how-to/use-graphql-for-management/)   | This example shows how to manage your application directory programmatically — full create, read, update and delete through a single GraphQL endpoint — with a guided UI that signs in using OAuth 2.0 (authorization code + PKCE) and an FDC3-driven config-as-code sync script.  |
| [How To Use OAuth Client Credentials](./how-to/use-oauth-client-credentials/)   | This example shows how a headless service calls HERE Cloud APIs with the OAuth 2.0 client credentials grant (HERE 15+). It includes a step-by-step connection check and a scheduled Audit API collector that pulls records by time window without gaps or duplicates.  |
