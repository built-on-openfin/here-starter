 ...
 */
let interopClients: Record<string, OpenFin.InteropClient> | undefined;
...
export async function init(excelSettings: ExcelSettings): Promise<void> {
	if (!settings) {
		...
		for (const contextGroup of contextGroups) {
			...
			await contextClient.addContextHandler((ctx) => {
				handleContext(contextGroup.id, ctx).catch((err) => {
					console.error(err);
				});
			});
 ...
 */
export async function launchExcel(
	assetInfo: OpenFin.AppAssetInfo,
	...
	if (
		...
		try {
			await fin.System.downloadAsset(assetInfo, (progress) => {
				...
				console.info(`Downloaded ${downloadedPercent}% of app asset with alias of ${assetInfo.alias}`);
			});
		} catch (error) {
...
async function getExcel(): Promise<ExcelApplication | undefined> {
	try {
		excel = await getExcelApplication(true);
		return excel;
	} catch (err) {
