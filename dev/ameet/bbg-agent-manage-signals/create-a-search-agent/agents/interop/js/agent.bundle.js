...
async function init(logger) {
    agentLogger = logger;
    const { intent, intent2, broadcast, dataSource } = await _openfin_cloud_api__WEBPACK_IMPORTED_MODULE_0__.Agent.getConfiguration();
    if (window.fdc3) {
        try {
            if ((0,_util__WEBPACK_IMPORTED_MODULE_1__.isStringValue)(intent)) {
            ...
            if ((0,_util__WEBPACK_IMPORTED_MODULE_1__.isStringValue)(intent2)) {
            ...
            if ((0,_util__WEBPACK_IMPORTED_MODULE_1__.isStringValue)(broadcast) && broadcast.toLowerCase() === "yes") {
            ...
            agentLogger?.error(`Error finding intent${intent ? `: ${intent}` : "Unknown"}: ${err.message}`);
        }
    }
    ...
    if (dataSource) {
        try {
            ...
            agentLogger?.error(`Error fetching user data: ${err.message}`);
        }
    }
    else {
...
async function onAction(action, result) {
    ...
    switch (actionName) {
        case "intent": {
            agentLogger?.info(`Raising primary intent: ${primaryIntent?.name} for user: ${resultData.id} - ${resultData.email}`);
            if (primaryIntent) {
                window.fdc3.raiseIntent(primaryIntent.name, context).catch((err) => {
                    agentLogger?.error(`Error raising primary intent: ${err instanceof Error ? err.message : String(err)}`);
                });
            }
            break;
        }
        case "intent2": {
            agentLogger?.info(`Raising secondary intent: ${secondaryIntent?.name} for user: ${resultData.id} - ${resultData.email}`);
            if (secondaryIntent) {
                window.fdc3.raiseIntent(secondaryIntent.name, context).catch((err) => {
                    agentLogger?.error(`Error raising secondary intent: ${err instanceof Error ? err.message : String(err)}`);
                });
            }
            break;
        }
    ...
    try {
        ...
        if (broadcastContext) {
            ...
            if (currentContextGroup) {
                ...
                agentLogger?.info("No current context group assigned so broadcast action will not be added even though it is enabled.");
            }
        }
 ...
 */
async function init() {
    const agent = await (0,_search__WEBPACK_IMPORTED_MODULE_1__.init)({
    ...
    const searchAgent = await _openfin_cloud_api__WEBPACK_IMPORTED_MODULE_0__.Agent.register(agent);
